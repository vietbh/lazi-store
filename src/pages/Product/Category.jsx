      ...
      <div key={product.id} className={`col-lg-3 col-sm-6`}>
        <div className={`product text-start bg-light mb-3 ${styles.borderProduct} ${styles.paddingImageProduct}`}>
          <div className="position-relative mb-3">
            ...
            <a className="d-block" to={product.product.slug}><img className={`img-fluid ${styles.borderImageProduct} ${styles.imageProduct} `} loading="lazy" src={product.image_url} data-src={product.image_url} alt={product.image_url}/></a>
          ...
          <div key={product.id} className="d-flex align-items-center mb-1">
            <p className="fw-bold m-1 text-danger" style={{fontSize:'14px'}}>{parseInt(product.price_sale).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}<span className="text-small">đ</span></p>
            <p className="fw-bold m-1 text-secondary text-truncate" style={{fontSize:'12px'}}>{parseInt(product.price_sale).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')}<span className="text-small">đ</span></p>
          </div>
